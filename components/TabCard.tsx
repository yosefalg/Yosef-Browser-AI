...
function pathOf(value:string){
  try{
    ...
    try{return decodeURIComponent(path).slice(0,72);}catch{return path.slice(0,72);}
  }catch{return '';}
}
...
export function TabCard({tab,viewMode,theme,busy,selected,selectionMode,recent,onOpen,onDuplicate,onClose,onMenu,onLongPress}:{tab:BrowserTab;viewMode:TabViewMode;theme:ThemePalette;busy:boolean;selected:boolean;selectionMode:boolean;recent:boolean;onOpen:()=>void;onDuplicate:()=>void;onClose:()=>void;onMenu:()=>void;onLongPress:()=>void}){
  ...
  const gridWidth=singleColumn?'100%':wide?'31.4%':'48.2%';
  const iconSize=singleColumn?48:compactPhone?42:46;
  return <Pressable disabled={busy} onPress={onOpen} onLongPress={selectionMode?onOpen:onLongPress} delayLongPress={320} style={({pressed})=>[
    ...
    singleColumn&&grid&&s.gridSingle,
    compactPhone&&grid&&!singleColumn&&s.gridCompact,
    narrowList&&!grid&&s.listCompact,
    ...
    {grid?<>
      <View style={s.gridTop}>
        <View style={s.siteWrap}><SiteIcon url={tab.url} size={iconSize} radius={14}/><View style={[s.securityBadge,{backgroundColor:theme.surface2,borderColor:theme.border}]}><Ionicons name={secure?'lock-closed':'warning-outline'} size={10} color={secure?theme.accent:theme.muted}/></View></View>
        {!selectionMode&&<View style={s.actions}>
          <Pressable onPress={event=>{event.stopPropagation();onMenu();}} hitSlop={12} style={[s.action,compactPhone&&s.actionCompact,{backgroundColor:theme.surface2,borderColor:theme.border}]} accessibilityRole="button" accessibilityLabel="خيارات التبويب"><Ionicons name="ellipsis-horizontal" size={18} color={theme.muted}/></Pressable>
          <Pressable onPress={event=>{event.stopPropagation();onClose();}} hitSlop={12} style={[s.action,compactPhone&&s.actionCompact,{backgroundColor:theme.surface2,borderColor:theme.border}]} accessibilityRole="button" accessibilityLabel="إغلاق التبويب"><Ionicons name="close" size={19} color={theme.muted}/></Pressable>
        </View>}
      </View>
      <View style={s.gridCopy}>
        <Text numberOfLines={singleColumn?1:2} style={[s.title,compactPhone&&s.titleCompact,{color:theme.text}]}>{tab.title||hostOf(tab.url)}</Text>
      ...
      {!selectionMode&&<View style={[s.actions,narrowList&&s.actionsCompactList]}>
        <Pressable onPress={event=>{event.stopPropagation();onMenu();}} hitSlop={12} style={[s.action,narrowList&&s.actionCompact,{backgroundColor:theme.surface2,borderColor:theme.border}]} accessibilityRole="button" accessibilityLabel="خيارات التبويب"><Ionicons name="ellipsis-horizontal" size={19} color={theme.muted}/></Pressable>
        <Pressable onPress={event=>{event.stopPropagation();onClose();}} hitSlop={12} style={[s.action,narrowList&&s.actionCompact,{backgroundColor:theme.surface2,borderColor:theme.border}]} accessibilityRole="button" accessibilityLabel="إغلاق التبويب"><Ionicons name="close" size={20} color={theme.muted}/></Pressable>
      </View>}
    </>}
  </Pressable>;
}
...
const s=StyleSheet.create({
  ...
  gridSingle:{minHeight:150,paddingHorizontal:15},
  gridCompact:{minHeight:184,padding:11,gap:8},
  ...
  listCompact:{minHeight:90,paddingHorizontal:11,paddingVertical:10,gap:9},
  ...
  siteWrap:{position:'relative',flexShrink:0},securityBadge:{position:'absolute',right:-4,bottom:-4,width:22,height:22,borderRadius:8,borderWidth:1,alignItems:'center',justifyContent:'center'},
  gridTop:{flexDirection:'row-reverse',justifyContent:'space-between',alignItems:'center',minWidth:0},
  actions:{flexDirection:'row',gap:7,flexShrink:0},actionsCompactList:{gap:5},action:{width:40,height:40,borderRadius:13,borderWidth:1,alignItems:'center',justifyContent:'center'},actionCompact:{width:36,height:36,borderRadius:12},
  gridCopy:{flex:1,justifyContent:'center',minWidth:0},copy:{flex:1,alignItems:'flex-end',minWidth:0},listTitleRow:{maxWidth:'100%',flexDirection:'row-reverse',alignItems:'center',gap:7},
  title:{fontSize:14,fontWeight:'900',textAlign:'right',lineHeight:20,flexShrink:1},titleCompact:{fontSize:13,lineHeight:18},host:{fontSize:10,textAlign:'right',flexShrink:1},path:{fontSize:9,marginTop:6,textAlign:'right',opacity:.8},compactRecent:{fontSize:8,fontWeight:'900',marginStart:3},
