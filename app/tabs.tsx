...
export default function TabsScreen(){
  ...
  const openTab=(tab:BrowserTab)=>router.replace({pathname:'/browser',params:{url:tab.url,tabId:String(tab.id)}});
  ...
  const newPrivate=()=>router.replace({pathname:'/browser',params:{privateMode:'1'}});
  const close=async(id:number)=>{await closeBrowserTab(id);await refresh();};
  const closeAll=async()=>{await closeAllBrowserTabs();await refresh();};
  ...
  return <SafeAreaView style={styles.root}>
    <View style={styles.header}>
      ...
      <Pressable onPress={newTab} disabled={busy} style={styles.add}><Text style={styles.addText}>＋</Text></Pressable>
    </View>

    <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      <View style={styles.actions}>
        <Pressable onPress={newTab} style={styles.primary}><Text style={styles.primaryText}>تبويب جديد</Text></Pressable>
        <Pressable onPress={newPrivate} style={styles.private}><Text style={styles.privateText}>تبويب خاص</Text></Pressable>
      </View>
      ...
      {tabs.length===0?<View style={styles.empty}><Text style={styles.emptyTitle}>لا توجد تبويبات مفتوحة</Text><Text style={styles.emptyText}>أنشئ تبويبًا جديدًا وسيبقى محفوظًا حتى تغلقه.</Text></View>:
        <View style={styles.grid}>{tabs.map(tab=><Pressable key={tab.id} onPress={()=>openTab(tab)} style={styles.card}>
          <View style={styles.cardTop}><View style={styles.favicon}><Text style={styles.faviconText}>{hostOf(tab.url).slice(0,1).toUpperCase()}</Text></View><Pressable onPress={()=>close(tab.id)} hitSlop={12} style={styles.close}><Text style={styles.closeText}>×</Text></Pressable></View>
