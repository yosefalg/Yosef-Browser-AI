...
export default function TabsScreen(){
  ...
  const refresh=useCallback(async()=>{
    try{
      ...
      setRefreshError('تعذر تحديث التبويبات الآن. لم نمسح البيانات المعروضة.');
    }
  },[]);
  ...
  return <View style={[s.fill,{backgroundColor:theme.bg}]}><SafeAreaView edges={['top','bottom','left','right']} style={s.root}>
    ...
    <ScrollView contentContainerStyle={s.content} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
      {!!refreshError&&<Pressable onPress={()=>void refresh()} accessibilityRole="button" accessibilityLabel="تعذر تحديث التبويبات، اضغط لإعادة المحاولة" style={[s.refreshError,{backgroundColor:theme.surface,borderColor:'#D98C80'}]}>
        <Ionicons name="warning-outline" size={19} color="#D98C80"/>
        <Text style={[s.refreshErrorText,{color:theme.text}]}>{refreshError}</Text>
        <Text style={[s.refreshRetry,{color:theme.accent}]}>إعادة</Text>
      </Pressable>}
