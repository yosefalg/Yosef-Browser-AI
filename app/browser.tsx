...
video.addEventListener('loadedmetadata',()=>{status.textContent='جاهز • '+(video.videoWidth||'?')+'×'+(video.videoHeight||'?');sync();show()});video.addEventListener('timeupdate',sync);video.addEventListener('play',sync);video.addEventListener('pause',sync);video.addEventListener('waiting',()=>status.textContent='جاري التخزين المؤقت…');video.addEventListener('playing',()=>{status.textContent='يعمل داخل RAID';show()});video.addEventListener('error',()=>{status.textContent='تعذر تشغيل المصدر مباشرة';flash('قد يحتاج هذا الموقع تشغيل الفيديو من صفحة المشاهدة نفسها.')});
const toggle=()=>video.paused?video.play().catch(()=>{}):video.pause();play.onclick=toggle;big.onclick=toggle;document.getElementById('back').onclick=()=>{video.currentTime=Math.max(0,video.currentTime-10);flash('رجوع 10 ثوانٍ')};document.getElementById('fwd').onclick=()=>{video.currentTime=Math.min(video.duration||Infinity,video.currentTime+10);flash('تقديم 10 ثوانٍ')};seek.oninput=()=>{if(Number.isFinite(video.duration)&&video.duration>0)video.currentTime=(Number(seek.value)/1000)*video.duration};speed.onclick=()=>{speedIndex=(speedIndex+1)%speeds.length;video.playbackRate=speeds[speedIndex];speed.textContent=speeds[speedIndex]+'×';flash('السرعة '+speeds[speedIndex]+'×')};document.getElementById('full').onclick=()=>{const el=document.documentElement;const fn=video.requestFullscreen||video.webkitRequestFullscreen||el.requestFullscreen||el.webkitRequestFullscreen;if(fn){try{fn.call(video.requestFullscreen||video.webkitRequestFullscreen?video:el)}catch{}}};pip.onclick=async()=>{try{if(document.pictureInPictureElement){await document.exitPictureInPicture();return}if(video.requestPictureInPicture){await video.requestPictureInPicture()}else flash('PiP غير مدعوم بهذا WebView')}catch{flash('تعذر تشغيل PiP لهذا المصدر')}};document.getElementById('download').onclick=()=>window.ReactNativeWebView?.postMessage('RAID_MEDIA_DOWNLOAD:'+src);document.body.addEventListener('click',show);video.play().catch(()=>{sync();status.textContent='اضغط تشغيل لبدء الفيديو'});show();
</script></body></html>`;
}
