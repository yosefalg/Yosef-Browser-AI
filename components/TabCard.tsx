...
export function TabCard({tab,viewMode,theme,busy,selected,selectionMode,recent,onOpen,onDuplicate,onClose,onMenu,onLongPress}:{tab:BrowserTab;viewMode:TabViewMode;theme:ThemePalette;busy:boolean;selected:boolean;selectionMode:boolean;recent:boolean;onOpen:()=>void;onDuplicate:()=>void;onClose:()=>void;onMenu:()=>void;onLongPress:()=>void}){
  ...
  const iconSize=singleColumn?50:compactPhone?42:46;
  return <Pressable disabled={busy} onPress={onOpen} onLongPress={selectionMode?onOpen:onLongPress} delayLongPress={320} style={({pressed})=>[
    ...
    {grid?<>
      ...
      {!selectionMode&&<View style={s.actions}>
        <Pressable onPress={event=>{event.stopPropagation();onMenu();}} hitSlop={12} style={[s.action,{backgroundColor:theme.surface2,borderColor:theme.border}]} accessibilityRole="button" accessibilityLabel="خيارات التبويب"><Ionicons name="ellipsis-horizontal" size={19} color={theme.muted}/></Pressable>
        <Pressable onPress={event=>{event.stopPropagation();onClose();}} hitSlop={12} style={[s.action,{backgroundColor:theme.surface2,borderColor:theme.border}]} accessibilityRole="button" accessibilityLabel="إغلاق التبويب"><Ionicons name="close" size={20} color={theme.muted}/></Pressable>
      </View>}
    </>}
  </Pressable>;
}
...
const s=StyleSheet.create({
  ...
  gridSingle:{minHeight:154,paddingHorizontal:15},
  ...
  actions:{flexDirection:'row',gap:7,flexShrink:0},action:{width:36,height:36,borderRadius:12,borderWidth:1,alignItems:'center',justifyContent:'center'},actionCompact:{width:32,height:32,borderRadius:11},
  ...
  title:{fontSize:14,fontWeight:'900',textAlign:'right',lineHeight:20,flexShrink:1},titleCompact:{fontSize:13,lineHeight:18},host:{fontSize:10,textAlign:'right',flexShrink:1},path:{fontSize:9,marginTop:6,textAlign:'right',opacity:.8},
