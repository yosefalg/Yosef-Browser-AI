...
export async function setPageContext(url: string, title: string, text: string) {
  const cleanText = text.slice(0, 16000);
  const now = Date.now();
  const d = await db();
  await d.runAsync('INSERT OR REPLACE INTO page_context (id,url,title,text,captured_at) VALUES (1,?,?,?,?)', url, title, cleanText, now);
  await upsertTabContext(url, title, cleanText, null);
}
...
export async function createAgentTask(title: string, payload: string, dueAt?: number | null) {
  const d = await db();
  const result = await d.runAsync(
    'INSERT INTO agent_tasks (title,payload,due_at,status,created_at) VALUES (?,?,?,?,?)',
    title.trim().slice(0, 160), payload.trim().slice(0, 4000), dueAt ?? null, 'pending', Date.now(),
  );
  return result.lastInsertRowId;
}

export async function getPendingAgentTasks(limit = 50) {
  const d = await db();
  return d.getAllAsync<AgentTask>('SELECT * FROM agent_tasks WHERE status=? ORDER BY COALESCE(due_at,created_at) ASC LIMIT ?', 'pending', limit);
}

export async function completeAgentTask(id: number) {
  const d = await db();
  await d.runAsync('UPDATE agent_tasks SET status=? WHERE id=?', 'done', id);
}

export async function createWorkspace(name: string) {
  const d = await db();
  const result = await d.runAsync('INSERT INTO workspaces (name,created_at) VALUES (?,?)', name.trim().slice(0, 120), Date.now());
  return result.lastInsertRowId;
}

export async function getWorkspaces() {
  const d = await db();
  return d.getAllAsync<Workspace>('SELECT * FROM workspaces ORDER BY created_at DESC');
}

export async function upsertTabContext(url: string, title: string, text: string, workspaceId: number | null) {
  const d = await db();
  await d.runAsync(
    `INSERT INTO tab_contexts (workspace_id,url,title,text,updated_at) VALUES (?,?,?,?,?)
     ON CONFLICT(url) DO UPDATE SET workspace_id=COALESCE(excluded.workspace_id,tab_contexts.workspace_id), title=excluded.title, text=excluded.text, updated_at=excluded.updated_at`,
    workspaceId, url, title, text.slice(0, 16000), Date.now(),
  );
}

export async function assignTabToWorkspace(url: string, workspaceId: number) {
  const d = await db();
  await d.runAsync('UPDATE tab_contexts SET workspace_id=? WHERE url=?', workspaceId, url);
}

export async function getTabContexts(workspaceId?: number | null, limit = 12) {
  const d = await db();
  if (workspaceId == null) {
    return d.getAllAsync<TabContext>('SELECT * FROM tab_contexts ORDER BY updated_at DESC LIMIT ?', limit);
  }
  return d.getAllAsync<TabContext>('SELECT * FROM tab_contexts WHERE workspace_id=? ORDER BY updated_at DESC LIMIT ?', workspaceId, limit);
}
