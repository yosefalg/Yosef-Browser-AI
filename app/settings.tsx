...
export default function SettingsScreen(){
  ...
  const services:SettingsRow[]=[
    ...
    {title:'RAID Security Center',hint:'فحص الروابط وكشف مؤشرات التمويه قبل الفتح',icon:'shield-lock-outline',route:'/security',badge:'جديد'},
  ...
  return <SafeAreaView edges={['top','bottom','left','right']} style={[s.root,{backgroundColor:theme.bg}]}>
    <View style={[s.head,{backgroundColor:theme.surface,borderBottomColor:theme.border}]}>
      ...
      <Pressable onPress={()=>router.push('/security')} style={({pressed})=>[s.iconButton,{backgroundColor:theme.surface2,borderColor:theme.border},pressed&&s.pressed]} accessibilityLabel="مركز الأمان"><MaterialCommunityIcons name="shield-lock-outline" size={21} color={theme.accent}/></Pressable>
    </View>
